...
async function getDriverInformationExchangeResponse(rdNumber, date) {
    ...
    try {     
        browser = await puppeteer.launch({
            ...
            args:  ['--no-sandbox', '--disable-setuid-sandbox',`--proxy-server=${grabAnProxyServer()}`],
            headless: true,
        ...
        let postStatusCode = await subQuerySteps(page, rdNumber, date)
        ...
        if(postStatusCode == 429) {
            ...
            postStatusCode = await subQuerySteps(page, rdNumber, date)
            if(postStatusCode == 429) {
